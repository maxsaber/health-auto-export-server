...
import {
  ...
  mapMetric,
  createMetricModel,
} from '../models/CustomMetric';
import { CustomMetricName } from '../models/CustomMetricName';
import { IngestData } from '../models/IngestData';
import { IngestResponse } from '../models/IngestResponse';

export const getCustomMetrics = async (req: Request, res: Response) => {
  try {
    const { from, to } = req.query;
    const selectedMetric = req.params.selected_metric as CustomMetricName;
    ...
    switch (selectedMetric) {
      case CustomMetricName.BLOOD_PRESSURE:
        metrics = await BloodPressureModel.find(query).lean();
        break;
      case CustomMetricName.HEART_RATE:
        metrics = await HeartRateModel.find(query).lean();
        break;
      case CustomMetricName.SLEEP_ANALYSIS:
...
export const saveCustomMetrics = async (ingestData: IngestData): Promise<IngestResponse> => {
  try {
    ...
    const saveOperations = Object.entries(metricsByType).map(([key, metrics]) => {
      switch (key as CustomMetricName) {
        case CustomMetricName.BLOOD_PRESSURE:
        ...
        default:
          ...
          const model = createMetricModel(key as CustomMetricName);
